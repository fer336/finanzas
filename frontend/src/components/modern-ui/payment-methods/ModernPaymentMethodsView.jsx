...
/**
 * ModernPaymentMethodsView — CRUD de métodos de pago, tema "Papel".
 ...
 * ModernCategoriesView (tabla mono uppercase header, hover #f0ead9) en
...
const ModernPaymentMethodsView = ({
  ...
  const data = paymentMethods.map((pm) => ({
    ...
    color: pm.color || pm.Color || '#5a7d52',
  ...
  return (
    <div>
      <div className="mb-4 flex items-start justify-between gap-3">
        <p className="text-[12.5px] text-[#8a8677] dark:text-[#93a0af]">Organizá cómo registrás cada movimiento.</p>
        <div className="flex shrink-0 items-center gap-2">
          <button
            type="button"
            onClick={refresh}
            disabled={isRefreshing}
            className="flex items-center gap-1.5 rounded-sm border border-[#ddd5c2] bg-white px-3 py-[7px] font-sans text-[13px] text-foreground transition-colors duration-150 hover:bg-[#f0ead9] disabled:opacity-50 dark:border-[#2e3844] dark:bg-[#212836] dark:hover:bg-[#212836]"
      ...
      <div className="mb-4 grid grid-cols-3 gap-2.5">
        ...
        <div className="rounded-sm border border-[#ddd5c2] bg-white p-3 dark:border-[#2e3844] dark:bg-[#212836]">
          <p className="text-[10.5px] uppercase tracking-[.06em] text-[#8a8677] dark:text-[#93a0af]">Activos</p>
          <p className="mt-1 font-mono text-[20px] font-semibold text-primary">{activos}</p>
        </div>
        <div className="rounded-sm border border-[#ddd5c2] bg-white p-3 dark:border-[#2e3844] dark:bg-[#212836]">
          <p className="text-[10.5px] uppercase tracking-[.06em] text-[#8a8677] dark:text-[#93a0af]">Inactivos</p>
          <p className="mt-1 font-mono text-[20px] font-semibold text-[#8a8677] dark:text-[#93a0af]">{inactivos}</p>
        </div>
      </div>
        ...
        <table className="w-full min-w-[640px]">
          <thead>
            <tr className="border-b-2 border-[#ddd5c2] dark:border-[#2e3844]">
              <th className="px-3.5 py-2.5 text-left font-mono text-[10.5px] uppercase text-[#8a8677] dark:text-[#93a0af]" style={{ letterSpacing: '.08em' }}>Método</th>
              ...
              <th className="hidden px-3.5 py-2.5 text-left font-mono text-[10.5px] uppercase text-[#8a8677] dark:text-[#93a0af] sm:table-cell" style={{ letterSpacing: '.08em' }}>Descripción</th>
          ...
          <tbody>
            {data.length === 0 ? (
              ...
              data.map((pm) => {
                ...
                return (
                  <tr key={pm.id} className="group border-b border-[#e7e0cf] transition-colors hover:bg-[#f0ead9] dark:border-[#2e3844] dark:hover:bg-[#212836]">
                    ...
                    <td className="px-3.5 py-2.5">
                      <span className="inline-flex rounded-full border border-[#ddd5c2] px-2 py-[2px] font-mono text-[10.5px] uppercase tracking-[.04em] text-[#5d6470] dark:border-[#2e3844] dark:text-[#93a0af]">
                        {formatTypeLabel(pm.tipo)}
                      </span>
                    </td>
                    <td className="hidden max-w-[240px] px-3.5 py-2.5 text-[12.5px] text-[#8a8677] dark:text-[#93a0af] sm:table-cell">
                    ...
                    <td className="px-3.5 py-2.5">
                      <span
                        className={`inline-flex rounded-full border px-2 py-[2px] font-mono text-[10.5px] uppercase tracking-[.04em] ${
                          pm.activo ? 'border-[#5a7d52] text-[#476442] dark:border-[#8fae7f] dark:text-[#8fae7f]' : 'border-[#ddd5c2] text-[#8a8677] dark:border-[#2e3844] dark:text-[#93a0af]'
                        }`}
                      >
                    ...
                    <td className="px-3.5 py-2.5">
                      <div className="flex items-center justify-end gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                        <button
                          ...
                          className="rounded-sm p-1.5 text-[#8a8677] transition-colors hover:bg-black/5 hover:text-foreground dark:text-[#93a0af]"
                        ...
                        <button
                          ...
                          className="rounded-sm p-1.5 text-[#8a8677] transition-colors hover:bg-black/5 hover:text-[#a04a34] dark:text-[#93a0af] dark:hover:text-[#c26a52]"
