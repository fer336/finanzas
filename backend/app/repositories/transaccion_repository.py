...
class TransaccionRepository:
        ...
        for t in transacciones:
            if t.metodo_pago:
                ...
                if tarjeta_nombre not in deuda_por_tarjeta:
                    deuda_por_tarjeta[tarjeta_nombre] = {
                        'nombre': tarjeta_nombre,
                        'icono': t.metodo_pago.icono,
                        'deuda': 0,
                        'cantidad_transacciones': 0
                    }
                deuda_por_tarjeta[tarjeta_nombre]['deuda'] += abs(t.monto)
                deuda_por_tarjeta[tarjeta_nombre]['cantidad_transacciones'] += 1
    ...
    def marcar_resumen_pagado(
        ...
        """Marcar un conjunto de transacciones como pagadas (pago de resumen)"""
        count = 0
        for transaccion_id in transaccion_ids:
            ...
            if transaccion and transaccion.es_credito and not transaccion.fecha_pago_real:
                transaccion.fecha_pago_real = fecha_pago
                transaccion.resumen_tarjeta_id = resumen_id
                count += 1
