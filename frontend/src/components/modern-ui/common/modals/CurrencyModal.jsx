    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4" style={{ background: 'rgba(32,36,44,.4)' }}>
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-xl border border-[#ddd5c2] bg-[#faf7ef]">
        <div className="sticky top-0 z-20 flex items-center justify-between border-b border-[#ddd5c2] bg-[#faf7ef] px-5 py-4 sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[12.5px] text-[#8a8677]">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 p-5 sm:p-6">
          {/* Vista previa */}
          <div className="flex items-center gap-3 rounded-sm border border-[#ddd5c2] bg-white px-4 py-3">
            <div className="rounded-sm bg-[#f0ead9] p-2 text-xl leading-none">{previewEmoji}</div>
            <div>
              <p className="font-mono text-[10px] uppercase tracking-[.08em] text-[#8a8677]">Vista previa</p>
              <p className="font-serif text-[15px] font-semibold text-foreground">
                {formData.codigo || 'XXX'} — {formData.nombre || 'Nombre'}
              </p>
              <p className="text-[12px] text-[#8a8677]">Símbolo: {formData.simbolo || '?'}</p>
            </div>
          </div>

          <div>
            <label className="mb-1.5 block text-[12.5px] font-medium text-[#5d6470]">Código ISO *</label>
            <input
              type="text"
            ...
            <p className="mt-1 text-[11.5px] text-[#8a8677]">Código de 3-10 letras mayúsculas (ej: USD, EUR, BTC, USDT)</p>
          </div>
          ...
          <div>
            <label className="mb-1.5 block text-[12.5px] font-medium text-[#5d6470]">Símbolo *</label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <select
                ...
                className="w-full rounded-sm border border-[#ddd5c2] bg-white px-3.5 py-2.5 text-[13.5px] text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
              >
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              type="submit"
              disabled={saving}
              className="flex flex-1 items-center justify-center gap-2 rounded-sm bg-primary px-4 py-2.5 text-[13.5px] font-semibold text-primary-foreground transition-colors duration-150 hover:bg-[#4f7047] disabled:opacity-50"
            >
