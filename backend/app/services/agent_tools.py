...
def get_monthly_summary(db: Session, year: int, month: int, usuario_id: str) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        gastos_credito = sum(
            abs(t['monto']) 
            for t in transacciones 
            if t['tipo'] == 'gasto' and t.get('es_credito', False) and not t.get('fecha_pago_real')
        )
...
def get_spending_by_category(db: Session, year: int, month: int, usuario_id: str) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        gastos = [t for t in transacciones if t['tipo'] == 'gasto' and not t.get('es_credito', False)]
        ...
        categorias_result = categoria_repo.get_all() 
        categorias = categorias_result.get('list', []) if isinstance(categorias_result, dict) else categorias_result
        ...
        for gasto in gastos:
            ...
            if cat_id:
                cat_nombre = categoria_map.get(cat_id, {}).get('nombre', 'Sin categoría')
...
def get_budget_status(db: Session, year: int, month: int, usuario_id: str) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        presupuestos_result = presupuesto_repo.get_all(usuario_id=usuario_id)
        presupuestos_lista = presupuestos_result.get('list', []) if isinstance(presupuestos_result, dict) else presupuestos_result
        ...
        for gasto in gastos:
            cat_id = gasto.get('categoria_id')
            if cat_id:
                gastos_por_cat[cat_id] = gastos_por_cat.get(cat_id, 0) + abs(gasto['monto'])
        ...
        for presupuesto in presupuestos:
            ...
            resultado.append({
                "nombre": presupuesto.get('categoria', {}).get('nombre', 'Presupuesto'),
...
def get_credit_card_expenses(db: Session, usuario_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        deuda = transaccion_repo.get_deuda_tarjetas(usuario_id=usuario_id)
...
def get_pending_payments(db: Session, usuario_id: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        result = pago_repo.get_all(usuario_id=usuario_id, limit=100)
    ...
    try:
        ...
        if categoria_nombre:
            ...
            for cat in categorias:
                if categoria_nombre.lower() in cat['nombre'].lower() or cat['nombre'].lower() in categoria_nombre.lower():
                    categoria_id = cat['id']
                    break
        ...
        if metodo_pago_nombre:
            metodos_result = metodo_repo.get_all()
            metodos = metodos_result.get('list', []) if isinstance(metodos_result, dict) else metodos_result
            for metodo in metodos:
                if metodo_pago_nombre.lower() in metodo['nombre'].lower() or metodo['nombre'].lower() in metodo_pago_nombre.lower():
...
def execute_tool(tool_name: str, arguments: Dict[str, Any], db: Session, usuario_id: str) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if tool_name == "get_monthly_summary":
            return get_monthly_summary(db, arguments['year'], arguments['month'], usuario_id)
        
        elif tool_name == "get_spending_by_category":
            return get_spending_by_category(db, arguments['year'], arguments['month'], usuario_id)
        
        elif tool_name == "get_budget_status":
            return get_budget_status(db, arguments['year'], arguments['month'], usuario_id)
