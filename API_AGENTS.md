...
- **Gasto fijo/recurrente (luz, alquiler, streaming)**: no hay un flag de recurrencia en `transacciones`. El patrón es crear la transacción normal (`POST /transacciones/`, `tipo: "gasto"`) para lo que ya se pagó, y por separado un `POST /pagos-pendientes/` con `recurrente: true`, `frecuencia_recurrencia: "semanal" | "mensual" | "anual"` y `fechavencimiento` seteada al próximo período — eso es lo que hace aparecer el próximo vencimiento en Vencimientos.
- **Préstamo recibido**: `POST /prestamos/` crea el préstamo Y automáticamente una transacción `tipo: "ingreso"` por `monto_prestado` (recibir un préstamo es plata que entra) — no hace falta crearla vos aparte. La respuesta incluye `transaccion_id`. Al hacer `DELETE /prestamos/{id}` esa transacción se borra también.
